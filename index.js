	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} Ja foi tarde 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD BOT DE SEKAI*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Anderson? Este é um recurso especial para o Anderson❌',
					ownerB: '[❗] Anderson? Este é um recurso especial para o Anderson❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de BOT DE SEKAI. Registre-se para fazer amizade com o bot de sekai por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Anderson|17\n\n──「 BOT DE SEKAI 」──`,
			...
			const ownerNumber = ["556993553851@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"556993553851@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["556993553851@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["556993553851@s.whatsapp.net"]//ubah nomor lo
			const premium = ["556993553851@s.whatsapp.net""] 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("anderson")){
			client.updatePresence(from, Presence.composing)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS BOT DE SEKAI 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Anderson domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Anderson ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT ANDERSON ✨* 」\n*│+ wa.me/5547992091566*╰──────*「 *ANDERSON* 」*────*\n\n*_SE QUER SER ADMIN DO BOT DE SEKAI_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/554792091566/*╰──────*「 * ANDERSON* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO BOT DE SEKAI_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* ANDERSON\n*WPP:* wa.me/556993553851\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
